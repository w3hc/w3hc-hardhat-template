...
describe("Basic ERC-20", function () {
  ...
  describe("Deployment", function () {
    it("Should return a balance of 10,000 units", async function () {
      const { basic, initialBalance, alice } = await loadFixture(deployContracts);
      expect(await basic.balanceOf(alice.address)).to.equal(initialBalance)
    })
  })
  ...
  describe("Interactions", function () {
    it("Should mint 1 unit", async function () {
      ...
      expect(await basic.balanceOf(alice.address)).to.be.equal(ethers.parseEther('10001'))
    });
    it("Should transfer 1 unit", async function () {
      ...
      expect(await basic.balanceOf(bob.address)).to.be.equal(ethers.parseEther('1'))
    })
  })
})